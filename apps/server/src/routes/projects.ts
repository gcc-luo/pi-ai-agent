...
export const projectsRoutes: FastifyPluginAsync = async (app) => {
  app.post("/", async (req, reply) => {
    ...
    if (description && description.length > 2000) return reply.code(400).send({ error: "description too long" });
    const p = app.projects.create({ id, name, workdir: absWorkdir, description });
