...
export class WeChatConversationRepository {
  constructor(private db: Database.Database) {}

  find(channelId: string, userId: string): WeChatConversation | null {
    const row = this.db.prepare(
      "SELECT channel_id, user_id, session_id, updated_at FROM wechat_conversations WHERE channel_id = ? AND user_id = ?",
  ...
  bind(channelId: string, userId: string, sessionId: string): void {
    this.db.prepare(`
      INSERT INTO wechat_conversations (channel_id, user_id, session_id, updated_at)
      VALUES (?, ?, ?, ?)
  ...
  touch(channelId: string, userId: string): void {
    this.db.prepare(
      "UPDATE wechat_conversations SET updated_at = ? WHERE channel_id = ? AND user_id = ?",
    ).run(Date.now(), channelId, userId);
  }

  list(channelId: string): WeChatConversation[] {
    return (this.db.prepare(
      "SELECT channel_id, user_id, session_id, updated_at FROM wechat_conversations WHERE channel_id = ? ORDER BY updated_at DESC",
