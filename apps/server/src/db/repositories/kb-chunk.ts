...
type Row = {
  ...
  page_end: number | null; content: string; char_count: number | null; created_at: number;
};
...
export class KbChunkRepository {
  ...
  insert(input: {
    ...
    const info = this.db.prepare(`
      INSERT INTO kb_chunks (kb_id, file_id, generation, seq, title_path, page_start, page_end, content, char_count, created_at)
      VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
    `).run(input.kbId, input.fileId, input.generation, input.seq, input.titlePath, input.pageStart, input.pageEnd, input.content, input.charCount, now);
